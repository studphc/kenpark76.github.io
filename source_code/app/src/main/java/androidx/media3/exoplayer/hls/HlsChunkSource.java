...
class HlsChunkSource {
    ...
    public void getNextChunk(LoadingInfo loadingInfo, long j, List<HlsMediaChunk> list, boolean z, HlsChunkHolder hlsChunkHolder) {
        ...
        long j6 = j4;
        this.trackSelection.updateSelectedTrack(j3, j6, j5, list, createMediaChunkIterators(hlsMediaChunk2, j));
        ...
        if (this.cmcdConfiguration != null) {
            ...
            CmcdData.Factory objectType = new CmcdData.Factory(this.cmcdConfiguration, this.trackSelection, Math.max(0L, j6), loadingInfo.playbackSpeed, CmcdData.Factory.STREAMING_FORMAT_HLS, !hlsMediaPlaylist.hasEndTag, loadingInfo.rebufferedSince(this.lastChunkRequestRealtimeMs), list.isEmpty()).setObjectType(getIsMuxedAudioAndVideo() ? CmcdData.Factory.OBJECT_TYPE_MUXED_AUDIO_AND_VIDEO : CmcdData.Factory.getObjectType(this.trackSelection));
            if (intValue == -1) {
                longValue = longValue == -1 ? -1L : longValue + 1;
            }
            SegmentBaseHolder nextSegmentHolder2 = getNextSegmentHolder(hlsMediaPlaylist, longValue, intValue == -1 ? -1 : intValue + 1);
        ...
        boolean shouldSpliceIn = HlsMediaChunk.shouldSpliceIn(hlsMediaChunk, uri, hlsMediaPlaylist, segmentBaseHolder, j2);
