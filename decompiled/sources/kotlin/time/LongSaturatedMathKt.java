...
public final class LongSaturatedMathKt {
    ...
    public static final long m1992saturatingAddNuflL3o(long j, DurationUnit unit, long j2) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        long m1909toLongimpl = Duration.m1909toLongimpl(j2, unit);
        if (((j - 1) | 1) == Long.MAX_VALUE) {
            return m1991checkInfiniteSumDefinedPjuGub4(j, j2, m1909toLongimpl);
        }
        if ((1 | (m1909toLongimpl - 1)) == Long.MAX_VALUE) {
            return m1993saturatingAddInHalvesNuflL3o(j, unit, j2);
        }
        long j3 = j + m1909toLongimpl;
        return ((j ^ j3) & (m1909toLongimpl ^ j3)) < 0 ? j < 0 ? Long.MIN_VALUE : Long.MAX_VALUE : j3;
    }

    /* renamed from: checkInfiniteSumDefined-PjuGub4  reason: not valid java name */
    private static final long m1991checkInfiniteSumDefinedPjuGub4(long j, long j2, long j3) {
        if (!Duration.m1895isInfiniteimpl(j2) || (j ^ j3) >= 0) {
            return j;
        }
    ...
    private static final long m1993saturatingAddInHalvesNuflL3o(long j, DurationUnit durationUnit, long j2) {
        long m1866divUwyO8pc = Duration.m1866divUwyO8pc(j2, 2);
        long m1909toLongimpl = Duration.m1909toLongimpl(m1866divUwyO8pc, durationUnit);
        return ((1 | (m1909toLongimpl - 1)) > Long.MAX_VALUE ? 1 : ((1 | (m1909toLongimpl - 1)) == Long.MAX_VALUE ? 0 : -1)) == 0 ? m1909toLongimpl : m1992saturatingAddNuflL3o(m1992saturatingAddNuflL3o(j, durationUnit, m1866divUwyO8pc), durationUnit, Duration.m1898minusLRDsOJo(j2, m1866divUwyO8pc));
    }

    private static final long infinityOfSign(long j) {
        int i = (j > 0L ? 1 : (j == 0L ? 0 : -1));
        Duration.Companion companion = Duration.Companion;
        return i < 0 ? companion.m1965getNEG_INFINITEUwyO8pc$kotlin_stdlib() : companion.m1964getINFINITEUwyO8pc();
    }

    public static final long saturatingDiff(long j, long j2, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        if ((1 | (j2 - 1)) == Long.MAX_VALUE) {
            return Duration.m1916unaryMinusUwyO8pc(infinityOfSign(j2));
        }
    ...
    private static final long saturatingFiniteDiff(long j, long j2, DurationUnit durationUnit) {
        ...
        if (((j3 ^ j) & (~(j3 ^ j2))) < 0) {
            if (durationUnit.compareTo(DurationUnit.MILLISECONDS) < 0) {
                ...
                Duration.Companion companion = Duration.Companion;
                return Duration.m1899plusLRDsOJo(DurationKt.toDuration((j / convertDurationUnit) - (j2 / convertDurationUnit), DurationUnit.MILLISECONDS), DurationKt.toDuration(j4, durationUnit));
            }
            return Duration.m1916unaryMinusUwyO8pc(infinityOfSign(j3));
        }
